...
一発勝負の場なので、従来の開幕キスティスカード回収を行うかも知れない。
...
- X-ATM092の50APで「力J(シヴァ)」「HPJ(イフリート)」「生命魔法精製(セイレーン)」を習得。
  - 「カード変化(ケツァクウァトル)」は、ドールで踏む歩数エンカのエリート兵を撃破したときに習得。
...
実は番号は、デリングシティのエレベーターを上った時点のフィールド乱数によって決定される。デリングシティへ向かう**電車の窓から見える電柱(pole)の数**で乱数の値がわかるので、それを元に番号を特定できる。
