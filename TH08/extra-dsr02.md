...
- [東方永夜抄 Extra RTA 要点](#東方永夜抄-extra-rta-要点)
  - [ゲーム・カテゴリについて(しゃべるネタがないときにでも)](#ゲーム・カテゴリについてしゃべるネタがないときにでも)
  - [マリス砲について(道中前半の途中にでも)](#マリス砲について道中前半の途中にでも)
  - [道中前半](#道中前半)
...
- カテゴリはExtra。以下が特徴。
  ...
  - 難易度は本編NormalとHardの間。
  - コンティニュー不可。
  - スペルカード展開中のボスには、ボム効果中ダメージが通らない。ボム連打による力押しが困難となっている。

## マリス砲について(道中前半の途中にでも)
- 普通にショットを撃つと「魔理沙のショット」が出る。低速移動ボタンを押すと操作キャラがアリスに切り替わり、ショットを撃つと「アリスのレーザー」が出る。
  - アリスのレーザーは低速移動をすぐ解除しても少しの間持続するため、低速移動ボタンをタイミングよく連打することによって、「魔理沙のショット」と「アリスのレーザー」を同時発生させることができ、火力が増加する。このテクニックを「マリス砲」と呼ぶ。
- マリス砲がその真価を発揮するのは、貫通性能を持つレーザーが多数の使い魔に当たる状況。使い魔に与えたダメージはその1/2が本体のボスに還元されるため、すさまじい速度でボスの体力が削られる。
  - 具体的には、終盤の「フジヤマヴォルケイノ」と「パゼストバイフェニックス」。このふたつが見せ場。

## 道中前半
- 「マリス砲」の使える詠唱組を使う。自機の選択と同時にタイマー開始。
...
- x軸を画面下のEnemyマーカーの「y」の右上に合わせておく
  - (Enemyマーカーはボスの位置を示す。移動するボスの正面をとるにはこれを参考にする)。
### 旧史「旧秘境史　‐オールドヒストリー‐」	
- 使い魔の完全固定弾幕と慧音の自機狙い弾が交互に展開される。
...
- 二つ前のオールドヒストリーと似た弾幕。
## 道中後半
- スペルカードでボムを撃つのは、「インペリシャブルシューティング(以下インペリ)」発動の防止が目的でもある。
  - スペルカードを合計7枚以上取得するとExtraステージの最後でインペリが発動するが、これは10sのロスになる。
  - そこで、スペルカード中にわざとボムを撃つことでスペルカードを落とし、取得枚数を6以下に抑えることでインペリを不発にさせる。
...
- 妹紅の「スペルカード」はボム無効。中ボス戦のようなマスタースパークでの力押しは通らない。
- 通常攻撃ではなるべく正面を維持するため、弾の発射音を数えることでボスの移動タイミングを把握している。
### 通常攻撃1
...
### 不死「徐福時空」
- 使い魔が弾を設置して「卍」型を作る弾幕。
- 弾にも使い魔にも当たらない半安全地帯にy軸を合わせる(画面最下段から上に3~4フレーム移動した高さ)。
### 通常攻撃5
### 滅罪「正直者の死」
