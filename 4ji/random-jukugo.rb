...
def main
    ...
    while !board.empty?
      ...
      [from[0], to[0]].uniq.each{|i|
        if jukugos_h.delete(board[i])
          # p [jukugos_h.size, jukugos_h.keys]
          board[i] = nil
        end
      }
      # 揃ったやつ(nil)を除去
  ...
  puts "stage\t%d (%d jukugos)" % [$option[:stage], STAGE_SLICE[$option[:stage]][1]]
  puts "N\t%d" % $option[:N]
...
# [0, 4, 6, 8, 12, 16]
SANJUKU_STAGE_SLICE = [[0, 0], [0, 4], [4, 6], [10, 8], [18, 12], [30, 16]]
# [0, 4, 6, 8, 10, 12, 14, 16]
YOJIJUKU_STAGE_SLICE = [[0, 0], [0, 4], [4, 6], [10, 8], [18, 10], [28, 12], [40, 14], [54, 16]]
